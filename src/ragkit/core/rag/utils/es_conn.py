...
logger = logging.getLogger('ragkit.es_conn')
...
class ESConnection():
    ...
    def delete_index(self, index_name: str) -> bool:
        """Drop an index. Returns True if it was deleted."""
    ...
    def list_indices(self, prefix: str = "") -> list[str]:
        """Return non-system indices, optionally filtered by name prefix."""
        resp = self.es.indices.get(index="*")
        return sorted(
            name for name in resp.keys()
            if not name.startswith(".") and (not prefix or name.startswith(prefix))
        )
